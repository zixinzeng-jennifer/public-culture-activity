...
class Hunan_yongzhou_whySpider(scrapy.Spider):
    ...
    # 爬去机构讲座活动所需的参数
    event_url = 'http://www.fcgqyg.cn/activity'
    ...
    def event_parse(self, response):
        ...
        while self.start_page <= self.end_page:
            driver.get(self.event_url + '?&page='+str(self.start_page))
            data = driver.page_source
            soup = BeautifulSoup(data, 'html.parser')
            # print(soup.prettify())
            article_lists = soup.find_all('div', {'class': 'new-list-4'})[0].find_all('li')
            for article in article_lists:
                item = CultureEventItem()
                try:
                    item['pav_name'] = '防城港市数字文化馆'
