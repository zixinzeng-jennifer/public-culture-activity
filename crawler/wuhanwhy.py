...
class WuhanwhySpider(scrapy.Spider):
    name = 'wuhanwhy'
    ...
    # 爬去机构讲座活动所需的参数
    event_url = 'http://www.whqy.cn/activity?&page=1'
    event_base_url = 'http://www.whqy.cn/activity?&page='
    ...
    def event_parse(self, response):
        ...
        data = response.body
        soup = BeautifulSoup(data, 'html.parser')
        article_lists = soup.find('div', {'class': 'new-list-3'})
        ...
        for article in article_lists.find_all('li'):
            ...
            try:
                ...
                item['url'] ='http://www.whqy.cn'+ article.div.a.attrs['href']
                item['activity_time'] = re.findall(r'(\d{4}-\d{1,2}-\d{1,2})', article.find('div', {'class': 'info'}).p.span.text)[0]
        ...
        if self.event_count_1 < self.event_page_end_1:
            ...
            yield scrapy.Request('http://www.whqy.cn/activity?&page=' + str(self.event_count_1)  + '.html', callback=self.event_parse)
        else:
            return None
    ...
    def event_text_parse(self, response):
        ...
        data = response.body
        soup = BeautifulSoup(data, 'html.parser')
        content = soup.find('div', {'style': 'display:block'}).find('div', {'class': 'booking-notice'})
        print(content)
        item['activity_name'] = soup.find('div', {'class': 'title-tag'}).h3.text.replace('\n','').replace(' ','')
